...
This CRD contains three bits of information:
 ...
 - any `--bind-interface` for the server
 - any `--usage-reporting-url` for the server
 - any `--bind-interface` for the server
 - any `--email` for the server
 - the status of the server (obtained by calling the list endpoint, `Status.Tunnels`)
 - whether or not this server is internal to the cluster

The one exception to this is if the `Spec.Location` is set to `External` in which case no deployment is created (the host is running outside the cluster).  Status `External` bastions are reconciled by `AquaductTaaS`.
...
This creates a `WeftGateway` that can be used for `Gateway`s.  It should only support features that are natively supported by `weft tunnel`, and it should manage many `WeftTunnel`s as required to serve the `Gateway`.

### `AquaductTaaS`

This CRD stores information about the user's online `aquaduct.dev` account.  Specifically, it stores the user's long-lived access token.  Then, by connecting to `api.aquaduct.dev`, it reconciles the following information into the cluster:
